...
// ChainHandler handles CRUD for WeaveChain.
type ChainHandler struct{ base }

func NewChainHandler(c client.Client, namespace string) ResourceHandler {
	return &ChainHandler{base{client: c, namespace: namespace}}
}
...
func (h *ChainHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.client.List(r.Context(), &list, client.InNamespace(h.namespace)); err != nil {
		internalError(w, r, err, "kind", "WeaveChain")
		return
	}
...
func (h *ChainHandler) Create(w http.ResponseWriter, r *http.Request) {
	var obj weavev1alpha1.WeaveChain
	...
	if err := h.client.Create(r.Context(), &obj); err != nil {
		...
		internalError(w, r, err, "kind", "WeaveChain", "name", obj.Name)
		return
	}
...
func (h *ChainHandler) Update(w http.ResponseWriter, r *http.Request) {
	name := nameFromURL(w, r)
	if name == "" {
		return
	}
	...
	if err := h.client.Update(r.Context(), &obj); err != nil {
		...
		internalError(w, r, err, "kind", "WeaveChain", "name", name)
		return
	}
	writeJSON(w, http.StatusOK, obj)
}

func (h *ChainHandler) Patch(w http.ResponseWriter, r *http.Request) {
	h.mergePatch(w, r, &weavev1alpha1.WeaveChain{})
}
