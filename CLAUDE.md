...
- `indexclient.FetchAppMetadata` (used by `stepOverrides`) parses `runner.args` as `map[string]string` — metadata.yaml must use map style (`args: {ENTRYPOINT: "main.py"}`), not a list of `{name, value}` objects. Required fields: `runner.port` (int32), `ingress.pathPrefix`, `resources.requests/limits` with standard Kubernetes quantity strings.
