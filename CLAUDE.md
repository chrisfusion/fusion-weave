...
- Testing panic recovery in `internal/trigger` goroutines: force a *real* panic (don't mock `recover()`) — e.g. a custom `cron.Schedule` whose `Next()` succeeds on an atomic-counted first call (satisfies the synchronous call inside `Upsert`'s initial heap build) then panics on a later call (reached only inside the runner's own goroutine). Assert via the `panicCh`/`TriggerPanic` report, not a crashed test binary. See `internal/trigger/panic_test.go`.
...
- Any CRD status field without `+optional` (e.g. `WeaveTriggerStatus.Active`, `WeaveChainStatus.Valid`, `WeaveJobTemplateStatus.Valid`, `WeaveServiceTemplateStatus.Valid`) is unsafe to write via a `MergeFrom`-diffed `Status().Patch()`: if the "new" value equals the field's Go zero value, the diff omits it, and on an object's first-ever status write this gets rejected with `status.<field>: Required value` — the reconcile then fails the same way forever. Use `r.Status().Update(ctx, obj)` for these instead — safe here because the object was freshly `r.Get()`'d earlier in the same Reconcile call (unlike the spec-`Update()` cache-staleness risk below, which is about reuse *across* reconciles). Found via BatchCron testing, fixed in `2baadb2` for `WeaveTriggerStatus.Active`; recurred in WeaveChain/WeaveJobTemplate/WeaveServiceTemplate, fixed alongside the panic-quarantine hardening work.
- WeaveRun objects created by `WeaveTriggerReconciler.createRun`/`createBatchRun`/`createKafkaRun` only get `fusion-platform.io/trigger` and `fusion-platform.io/chain` labels (`createBatchRun` additionally sets `fusion-platform.io/batch-job-id`) — a custom label on the WeaveTrigger does NOT propagate to its created WeaveRuns. Query created runs with `-l fusion-platform.io/trigger=<name>`, not a copied label.
...
- Both `config/rbac/role.yaml` AND `deployment/fusion-weave/templates/role.yaml` are hand-maintained — `make generate` does NOT update either; edit both manually when CRD resource names or API group change. Same pair-maintenance applies to every other RBAC file (`backup-role.yaml`, `api-role.yaml`, etc.) — grep for the resource/kind across both `config/rbac/` and `deployment/fusion-weave/templates/` before assuming one edit is enough.
- `internal/trigger`'s schedulers (`CronScheduler`/`BatchCronScheduler`/`KafkaConsumer`) are inherently concurrent — always run `go test ./internal/trigger/... -race`, not plain `go test`. A real data race (a plain read racing a mutex-guarded write) passed a normal test run cleanly and was only caught by `-race`.
- To test a status-gated reconcile branch without reproducing the real condition that sets it (e.g. WeaveTrigger quarantine after a panic): `kubectl patch <kind> <name> -n fusion --subresource=status --type=merge -p '{"status":{...}}'` writes directly to the status subresource, bypassing spec validation.
...
kubectl annotate weavetrigger <name> fusion-platform.io/reset=true --overwrite -n fusion   # clear status.quarantined after a panic and re-register the activation source; one-shot
...
- **Why this matters beyond staleness**: a deployment stuck on an old binary doesn't just miss new features — it silently drops any spec field added to a Go type since that build, on every Create/Update, with no error (unknown JSON keys are dropped by `encoding/json` before the object ever reaches the API server). Confirmed 2026-09-10: `WeaveJobTemplateSpec.CodeSource` vanished on write until the operator/api-server were rebuilt from current source. If a field mysteriously isn't persisting, check the deployed image tag/age before debugging CRD schemas or handler code.
...
- Adding a new `WeaveTriggerType`: (1) add constant + extend kubebuilder enum in `weavetrigger_types.go`, add config struct + field on `WeaveTriggerSpec`; (2) `make generate` + copy CRDs; (3) define `XxxFireRequest` + consumer/scheduler in `internal/trigger/` with `Upsert`/`Remove`/`Stop`; (4) add field + `pendingXxxFires` map + `drainXxxFireChannel` goroutine to reconciler; (5) add `consumePendingXxxFires`, `syncXxxSource`, `maybeCreateXxxRun`, `createXxxRun`; (6) case in `syncActivationSources` + `Remove(key)` in NotFound path; (7) wire channel + consumer in `cmd/main.go` + update `NewWeaveTriggerReconciler` args; (8) REST handler + route in `router.go`. (9) If the new scheduler/consumer runs its own goroutine per trigger (like `CronScheduler`/`BatchCronScheduler`/`KafkaConsumer`), it must recover panics itself and report via `trigger.TriggerPanic` on a `panicCh` — these goroutines run outside controller-runtime's per-Reconcile panic recovery, so an unrecovered panic there crashes the whole operator process, not just one trigger.
- Panic isolation for activation sources: `CronScheduler`/`BatchCronScheduler`/`KafkaConsumer` each recover panics inside their own per-trigger goroutine, self-unregister (so that trigger stops firing immediately), and report a `trigger.TriggerPanic{Namespace,Name,Source,Reason}` on the shared `triggerPanicCh` (buffered, non-blocking send — a full channel drops the report but the goroutine has already unregistered either way). `WeaveTriggerReconciler.drainPanicChannel` consumes it and sets `Status.Quarantined=true` (distinct from `Spec.Paused`, which is user-set) — `syncActivationSources` is skipped for a quarantined trigger on every subsequent reconcile until the `fusion-platform.io/reset` annotation is applied. Deliberately no auto-retry: a panic is a defect, unlike the already-self-healing chain-not-found/invalid case (`setInactive`, which now also persists `Status.InactiveReason`).
- The self-unregister step above must compare by *instance identity* (the runner/entry pointer, or a per-invocation generation token), never by map key alone — a concurrent `Upsert(key, ...)` can install a new runner under the same key while an old one is still unwinding from a panic; deleting by key would let that late panic remove the new, live runner instead. Build the identity check (e.g. the closure that becomes `onPanic`) fully *before* starting the goroutine, not by passing a value that gets set after — the latter is a data race (`go test -race` catches it; a plain `go test` run does not).
- `handler.EnqueueRequestsFromMapFunc` callbacks (the `Watches(...)` closures in each controller's `SetupWithManager`) run in the client-go informer's own goroutine, not inside a `Reconcile` call — so they are NOT covered by controller-runtime's per-Reconcile panic recovery either, and since client-go re-panics after `HandleCrash` logs it, an unrecovered panic there crashes the whole process for every object in the cluster, not just the reconciled one. All such callbacks in this repo are wrapped with `internal/controller.safeMapFunc` (recovers, logs, returns no requests) — wrap any new one the same way.
- `WeaveTriggerStatus.Active` has no `+optional` marker, so the CRD requires it. If `syncXxxSource` (step 5 above) does its own status write — as `syncBatchCronSource` does for `batchJobCount`/`batchJobErrors`, still via a `MergeFrom`-diffed `Patch()` — prefer `r.Status().Update()` instead (see the required-status-field gotcha above) for any new `syncXxxSource`, or a brand-new trigger's first reconcile can deadlock permanently on `status.active: Required value`. `syncBatchCronSource` itself gets away with `Patch()` only because it always sets `Active = true`, a genuine diff from the zero value — don't copy that pattern for a field that could legitimately stay at its zero value.
...
- `batch/v1` Job and `v1` Service object names must be ≤63 bytes, not the generic 253-byte object-name limit — Kubernetes auto-derives a pod-template label from the name (`job-name` for Jobs; selector/endpoint labels for Services), and label values are capped at 63 bytes. `jobbuilder.JobName` and every `deploybuilder` step-resource-name function (`DeploymentName`/`ServiceName`/`IngressName` + `Run*` counterparts) route through `codesource.TruncateK8sName(name, 63)` (hash-suffix truncation) for exactly this reason — any new name-generation helper for these resource kinds must do the same, not just cap at 253.
