...
- `externalAuth.serviceAccounts`/`externalAuth.oidcSecrets`/`externalAuth.defaultTokenTTL`/`externalAuth.saAudience` Helm values — deploy-time allowlists (up to 5 entries each) plus a configurable `aud` claim (default `"fusion"`) for `ExternalAuthRef`'s `serviceAccount` mode, wired into both the operator and API server deployments (API server plumbing is config-only for now, no query endpoint yet).
