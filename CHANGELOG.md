...
- `AuthSecretRef` is now also mounted unconditionally as a file/directory (`/var/run/secrets/fusion-platform.io/auth-secret/`, one file per Secret key) into every step pod, Job and Deploy kind alike, in addition to its existing (now `unsafeEnvironmentInjector`-gated) `envFrom` injection.
- `externalAuth.serviceAccounts`/`externalAuth.oidcSecrets`/`externalAuth.defaultTokenTTL`/`externalAuth.saAudience` Helm values — deploy-time allowlists (up to 5 entries each) plus a configurable `aud` claim (default `"fusion"`) for `ExternalAuthRef`'s `serviceAccount` mode, wired into both the operator and API server deployments.
- `GET /api/v1/external-auth/options` — read-only endpoint (viewer role) returning the configured `serviceAccounts`/`oidcSecrets` allowlists as JSON arrays, so a GUI building `externalAuthRef` can populate a name picker per mode without hardcoding the allowlist. Echoes the static config as-is, no cluster lookups.
...
### Changed
- Renamed `metadata.yaml`'s `ingress.pathPrefix` field to `ingress.path` (and the injected `WEAVE_INGRESS_PATH_PREFIX` env var to `WEAVE_INGRESS_PATH`) to make clear it configures a URL path segment for the artifact's own app, not a hostname/subdomain component — hostnames remain exclusively controlled by `WeaveIngressRule.name`/`WeaveRunStepOverride.ingressName`. No back-compat alias; artifacts using the old key must republish `metadata.yaml`.
- `internal/jobbuilder.Build` gains three new parameters (`unsafeEnvironmentInjector`, `externalAuthRef`, `externalAuthSecretName`) to support the new external-auth mounting/env-injection logic.
- `internal/deploybuilder.Build`/`BuildFromOverride` each gain one new parameter (`unsafeEnvironmentInjector`) so `AuthSecretRef`'s new file-mount/envFrom-gating behavior applies consistently to Deploy-kind steps too.
...
### Fixed
- `deploybuilder.BuildIngressFromOverride`'s ingress path derived from `metadata.yaml` was used raw (e.g. an explicit `"/"` produced a malformed `"//"` Ingress path); now normalized via the new `indexclient.NormalizeIngressPath` (empty or `"/"` both mean root `"/"`, otherwise a single leading `"/"`).
...
- `WeaveChainReconciler`/`WeaveJobTemplateReconciler`/`WeaveServiceTemplateReconciler`: a brand-new invalid object (e.g. a chain step referencing a missing template) could deadlock permanently on `status.valid: Required value` — same root cause as the WeaveTrigger `status.active` incident, since `status.valid` also has no `+optional` marker. Fixed the same way, by switching to `r.Status().Update()`.
- `internal/monitoring/handlers/runs.go`: `RunsHandler.seenRuns` was written from concurrent HTTP request goroutines with no synchronization — two overlapping `GET /monitor/v1/runs` requests racing a cache miss could hit Go's fatal "concurrent map writes" panic and crash the API server process. Now guarded by a mutex.
- `weaverun_controller.go` `handleRunDeletion`: removed the deploy-cleanup finalizer via `r.Update()` instead of `client.MergeFrom`+`r.Patch()`, unlike every other finalizer call site in the file — risked silently erasing a newly-added spec field if the informer cache still held a pre-upgrade copy of the object during a rolling operator upgrade.
- `internal/trigger/{cron,batchcron,kafka}.go`: the panic-quarantine goroutines added above could, in narrow windows, remove or lose updates for the wrong generation of a trigger's runner (e.g. a late panic from a superseded schedule deleting the newly-installed one) if a concurrent `Upsert` raced it. Panic cleanup now compares runner/entry identity, not just the trigger key.
- `internal/trigger/webhook.go`: the webhook handler blocked indefinitely sending to a full `FireCh` (no non-blocking guard, unlike the Kafka/BatchCron sources) — under sustained load with a lagging reconciler this accumulated one blocked goroutine per request with no bound. Now responds `503` instead of blocking.
- `internal/backup`: daily backups never captured the ConfigMap a `BatchCron`-type WeaveTrigger depends on (`spec.batchCron.jobsConfigMapRef`), so restoring one left it permanently unable to reconcile ("ConfigMap ... not found" on every attempt). Now captured (deduplicated) and restored alongside it.
- `weaverun_controller.go` `registerRunActiveDeployment`: never set `CodeSourceDeployedVersion` for a run-owned (stepOverrides) deploy step, unlike the chain-owned path — caused an unconditional, unnecessary Deployment rolling restart on the very next code-source poll after the step became Deployed.
- `internal/trigger.SanitizeJobID`: used `unicode.IsLetter`/`IsDigit`, admitting non-ASCII Unicode letters/digits into a value used as both a Kubernetes label and a `WeaveRun` `GenerateName` prefix — the API server then rejects a job whose ID contains one, failing that BatchCron entry. Restricted to ASCII, matching `codesource.WritableVolumeName`'s DNS-1123 convention.
- `internal/apiserver/handlers/batchtriggers.go`: a failed rollback `Delete` (after ConfigMap creation failed) discarded its own error silently — a double failure left an orphaned, undiagnosable WeaveTrigger with no trace in the logs. Now logged explicitly.
...
- `showroom.codeSourceApps.batchMetadata` (`codesource-batchcron.yaml`) — the combined `codeSource` + `BatchCron` showroom example: a Job-kind step pulls the `app.batch-metadata-reader` artifact via `codeSource` and is fired on a schedule by a `BatchCron` trigger (`showroom-batch-metadata-jobs` ConfigMap), reading back both its own artifact metadata (`WEAVE_*`) and the firing job's metadata (`JOB_*`/`JOB_METADATA`) at runtime.

### Changed
- **Breaking (Helm values)**: `samples.*` renamed to `showroom.*`. `samples.enabled=true` no longer has any effect; use `showroom.enabled=true` (equivalent Tier 1 behavior is `showroom.chains.sharedStorage=true`, the default). `samples.sharedStorage.*` moved to `showroom.sharedStorage.*` unchanged.
